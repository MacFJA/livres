...
build:
	rm -rf build/app
	mkdir -p build/app
	cp -Rp .make _extra assets bin config src templates node_modules vendor .env composer.json composer.lock Makefile package.json symfony.lock webpack.config.js package-lock.json LICENSE.md build/app/
